...
a token is used to authenticate a users session. it is provided by the server after a
successful login

- [ ] What steps can you take in your web apps to keep your data secure?

by requiring a login to receive a token, we can then have a protected area of
our app that sends the authorization token to the server to pull in protected
data. We can also use the token as a way of validating the the user should have
access to a particular section of the website and redirect them to the login
page if the token does not exist.

- [ ] Describe how web servers work.

web servers are software applications that expose specific ports to the
internet. When an http request is made to them on those ports they respond with
the appropriate resourses. For a rest API this usually takes the form of CRUD
requests. For a traditional web server, when a request is made to it's IP
address it will serve up the index.html and any other requested resourses.
